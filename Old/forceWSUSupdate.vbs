strScriptVer = "2.5 - Rob Dunn (uphold2001@hotmail.com)"
'~~[author]~~
'Rob Dunn
...
'Full EXE path to WindowsUpdateAgent20-x86.exe.  It will install it silently if 
' the PC needs it
sExePath = Chr(34) & "\\houmwinetop01\WsusContent\WindowsUpdateAgent30-x86.exe" & Chr(34)
...
If Err.Number = 0 Then 
  ...
  If replace(updateInfo,strLocaleDelim,"") = replace(strWUAgentVersion,strLocaleDelim,"") then
    ...
    bUpdateNeeded = False 
  ElseIf cint(replace(updateInfo,strLocaleDelim,"")) < cint(replace(strWUAgentVersion,strLocaleDelim,"")) then
